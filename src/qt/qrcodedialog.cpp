...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("RSC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("researchcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non RSC unit input in lnReqAmount, we generate the URI with RSC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::RSC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
